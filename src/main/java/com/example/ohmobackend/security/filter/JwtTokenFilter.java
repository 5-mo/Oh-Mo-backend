...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    private void setJsonResponse(HttpServletResponse response, int statusCode, BaseErrorCode code)
            throws IOException {
        response.setStatus(statusCode);
        response.setContentType("application/json;charset=UTF-8");
        String jsonResponse = String.format("{\"isSuccess\": false, \"code\": \"%s\", \"message\": \"%s\"}", code.getReasonHttpStatus().getCode(), code.getReasonHttpStatus().getMessage());
